...
class Search(Tk):
    def __init__(self):
        ...
        self.maxsize(800,668)
        self.minsize(800,500)
        self.canvas = Canvas(self,width=800, height=600)
        self.img = ImageTk.PhotoImage(Image.open("res/book-review-01.jpg"))
        self.canvas.create_image(0, 0,anchor=NW, image=self.img)
        self.canvas.pack(fill = BOTH)
        self.iconbitmap(r'libico.ico')
        l1=Label(self,text="Search Library",bg='#3888FF', font=("Courier new",20,'bold')).place(x=290,y=20)
        l = Label(self, text="Search By",bg='#B3FFDB', font=("Courier new", 15, 'bold')).place(x=60, y=96)
        def insert(data):
        ...
        def ge():
            if (len(g.get())) == 0:
                ...
                try:
                    ...
                    messagebox.showerror("Error","Something goes wrong")
        b=Button(self,text="Find",width=15,bg='#3333FF',font=("Courier new",10,'bold'),command=ge).place(x=460,y=148)
        ...
        la = Label(self, text="Enter",bg='#B3FFDB', font=("Courier new", 15, 'bold')).place(x=100, y=150)
