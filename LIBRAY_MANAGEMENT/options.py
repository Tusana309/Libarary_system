...
class MainWin(Tk):
    def __init__(self):
        ...
        self.img = ImageTk.PhotoImage(Image.open("res/book.jpg"))
        cavas = Canvas(self)
...
#calling scripts
        def a_s():
            os.system('%s' % ('Add_Student.py'))

        def a_b():
            os.system('%s' % ('Add_Books.py'))

        def r_b():
            os.system('%s' % ('remove_book.py'))

        def r_s():
            os.system('%s'  % ( 'Remove_student.py'))

        def ib():
            os.system('%s' % ('issueTable.py'))

        def ret():
            os.system('%s ' % ( 'ret.py'))

        def sea():
            os.system('%s' % ('Search.py'))
        ...
        def ser():
            if(len(self.studid.get())==0):
             ...
             try:
                ...
                cursor = conn.cursor()
                changeId = int(self.studid.get())
                # changeName = self.entry.get()

                cursor.execute("Select bi.issue_id,s.name,b.name,bi.issue_date,bi.return_date from issue_book bi,student s,book b where s.stud_id = bi.stud_id and b.book_id = bi.book_id and s.stud_id = %s",[changeId])
                pc = cursor.fetchall()
                if pc:
        ...
        def check():
            try:
                ...
                if not z:
                    ...
                    #label and input box
                    self.label3 = Label(self, text='LIBRARY MANAGEMENT SYSTEM',fg='black',bg="#CCFFFF" ,font=('Courier new', 30, 'bold'))
                    self.label3.place(x=350, y=22)
                    self.label4 = Label(self, text="ENTER STUDENT ID",bg="#FFB691", font=('Courier new', 18, 'bold'))
                    ...
                    self.label5 = Label(self, text="ENTER THE BOOK ID",bg="#FFB691", font=('Courier new', 18, 'bold'))
                    ...
                    self.label6 = Label(self, text="INFORMATION DETAILS",bg="#CCFFFF",  font=('Courier new', 15, 'underline', 'bold'))
